...
contract CrowdTank {
    ...
    event InvestmentWithdrawn(address indexed funder, uint projectId, uint amount);
    ...
    function withdrawInvestment(uint _projectId) public {
    require(block.timestamp < projects[_projectId].deadline, "Deadline passed");
    require(contributions[_projectId][msg.sender] > 0, "No funds to withdraw");
    uint amount = contributions[_projectId][msg.sender];
    contributions[_projectId][msg.sender] = 0;
    payable(msg.sender).transfer(amount);
    emit InvestmentWithdrawn(msg.sender, _projectId, amount);
}
}
